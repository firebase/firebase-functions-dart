...
///
/// [nativeAssetsAvailable] selects which startup command the endpoints get.
String generateManifestYaml(
  Map<String, ParamSpec> params,
  Map<String, EndpointSpec> endpoints, {
  bool nativeAssetsAvailable = false,
}) {
  final map = _buildManifestMap(
    params,
    endpoints,
...
Map<String, dynamic> _buildManifestMap(
  Map<String, ParamSpec> params,
  Map<String, EndpointSpec> endpoints, {
  required bool nativeAssetsAvailable,
}) {
  ...
  if (endpoints.isNotEmpty) {
    manifest['endpoints'] = {
      for (final endpoint in endpoints.values)
        toCloudRunId(endpoint.name): _buildEndpointMap(
          endpoint,
          nativeAssetsAvailable: nativeAssetsAvailable,
        ),
    };
...
/// Cloud Run startup command for `dart compile exe` builds.
/// Must match where the Firebase CLI writes the executable.
const dartCompileExeCommand = './bin/server';

/// Cloud Run startup command for `dart build cli` builds.
/// Must match where the Firebase CLI writes the bundle.
const dartBuildCliCommand = './build/cli/linux_x64/bundle/bin/server';

/// Builds a single endpoint entry as a map.
Map<String, dynamic> _buildEndpointMap(
  ...
  final command = nativeAssetsAvailable
      ? dartBuildCliCommand
      : dartCompileExeCommand;
  map['command'] = [command];
